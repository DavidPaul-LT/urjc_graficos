...
// Luz dinámica
vec3 lposdynamic = vec3(2.0 * sin(uTime)+4.0, sin(uTime)/2.0, 2.0 * cos(uTime)+5.0);
...
vec3 shade()
{
    ...
    vec3 dynamicL = normalize(lposdynamic - posWorld); // Vector hacia la luz
    float dynamicDistance = length(lposdynamic - posWorld);
    ...
    vec3 dynamicV = normalize(cameraPos-posWorld);
